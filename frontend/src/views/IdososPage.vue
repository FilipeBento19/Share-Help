...
onMounted(async () => {
  ...
  // Buscar instituições
  try {
    console.log('🎯 Buscando instituições (IdososPage)...')
...
// Instituição recomendada (Lar Renascer)
const recommendedInstitution = computed(() => {
  ...
  const ong = ongs.value.find((o) => o.id === 'lar-renascer')
...
// Instituições disponíveis (categoria idosos, exceto a recomendada)
    ...
    <div v-else>
      ...
      <section class="instituicoes-disponiveis">
        ...
        <div v-if="availableInstitutions.length === 0" class="empty-institutions">
          <p>Nenhuma instituição adicional encontrada na categoria idosos.</p>
        </div>
