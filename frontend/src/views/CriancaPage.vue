...
onMounted(async () => {
  ...
  // Buscar instituições
  try {
    console.log('🎯 Buscando instituições (CriancaPage)...')
...
// Instituição recomendada (Ecos de Esperança)
const recommendedInstitution = computed(() => {
  ...
  const ong = ongs.value.find((o) => o.id === 'ecos-esperanca')
...
// Instituições disponíveis (categoria crianças, exceto a recomendada)
      ...
      <motion.div :initial="{ opacity: 0, scale: 0.95 }" :animate="{ opacity: 1, scale: 1 }"
        ...
        <section class="instituicoes-disponiveis">
          ...
          <div v-if="availableInstitutions.length === 0" class="empty-institutions">
            <p>Nenhuma instituição adicional encontrada na categoria crianças.</p>
          </div>
